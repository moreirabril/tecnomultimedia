class Juego {
  ...
  Juego () {
    ...
    boton3=new Boton ("SIGUIENTE", 900, 510, 300, 100);
  ...
  void dibujar() {
    if (estado=="inicio" ) {
      image(inicio, width/2, height/2, 1280, 720);
      boton1.dibujar();
    } 
    if (estado=="jugando") {
      image(fondo, width/2, height/2, 1280, 720);
      for (int i=0; i<cantidadE; i++) {
        enemigo[i].dibujar();
      }
      puerta0.dibujar();
    ...
    if (estado=="morir") {
      image(muerte, width/2, height/2, 1280, 720);
      boton2.dibujar();
    }

    if (estado=="ganar") {
      image(ganar, width/2, height/2, 1280, 720);
      boton3.dibujar();
      textSize(40);
  ...
  void presionar () {
    if (estado=="inicio" && mouseX > 150 && mouseX < 150+300 && mouseY > 50 && mouseY < 50+100 ) { //(450,250,300,100)
      boton1.mouseEncima(); 
      estado="jugando";
    } else if (estado=="morir" && mouseX > 500 && mouseX < 500+300 && mouseY > 440 && mouseY < 440+100 ) { //(450,460,300,100)
      boton2.mouseEncima(); 
      estado="inicio";
    }
  }
